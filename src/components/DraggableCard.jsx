...
const DraggableCard = ({ 
  ...
  const { percentage, isCompleted, categoryColor, categoryGradient } = useMemo(() => ({
    percentage: Math.min((goal.currentProgress / goal.targetValue) * 100, 100),
    isCompleted: Math.min((goal.currentProgress / goal.targetValue) * 100, 100) >= 100,
    categoryColor: categoryColors[goal.category] || categoryColors.personal,
    categoryGradient: categoryGradients[goal.category] || categoryGradients.personal
  }), [goal.currentProgress, goal.targetValue, goal.category])
  ...
  const handleProgressUpdate = useCallback((increment) => {
  ...
  }, [goal.currentProgress, goal.targetValue, goal.id, onUpdateProgress])

  const handleDrag = useCallback((event, info) => {
    // Update local drag offset without triggering position changes
  ...
  const handleDragEnd = useCallback((event, info) => {
    ...
    onPositionChange(goal.id, newPosition)
  }, [position.x, position.y, goal.id, onPositionChange])
  ...
  return (
    <motion.div
      ...
      animate={{
        x: isDragging ? position.x : position.x,
        y: isDragging ? position.y : position.y,
      ...
      className={`absolute cursor-move select-none p-3 sm:p-4 rounded-2xl shadow-lg border transition-all duration-300 backdrop-blur-sm overflow-hidden group w-60 sm:w-64 lg:w-72 h-70 sm:h-72 lg:h-80 flex flex-col touch-none ${
        isCompleted
          ? `bg-gradient-to-br ${categoryGradient} text-white border-white/20`
          : darkMode 
            ? `bg-gradient-to-br from-gray-800/95 via-gray-800/90 to-gray-900/95 border-gray-700/50`
            : `bg-gradient-to-br from-white/95 via-white/90 to-gray-50/95 border-gray-200/50`
      } ${justCompleted ? 'animate-pulse ring-4 ring-green-400/50' : ''}`}
      style={{
        boxShadow: isDragging 
          ? `0 20px 40px rgba(0,0,0,0.3), 0 0 30px ${categoryColor}50, 0 0 60px ${categoryColor}30`
          : isCompleted 
            ? undefined 
            : `0 0 20px ${categoryColor}40, 0 0 40px ${categoryColor}20, 0 4px 15px rgba(0,0,0,0.1)`
      }}
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        <div className={`absolute top-6 right-6 w-4 h-4 rounded-full opacity-20 ${
          isCompleted ? 'bg-white' : `bg-gradient-to-br ${categoryGradient}`
        }`} />
        ...
        <div className="grid grid-cols-2 gap-1">
          {[...Array(6)].map((_, i) => (
            <div key={i} className={`w-1 h-1 rounded-full ${
              isCompleted ? 'bg-white/60' : darkMode ? 'bg-gray-400' : 'bg-gray-600'
            }`} />
          ))}
        </div>
      </div>
      ...
      {/* Progress Circle */}
      <div className="flex justify-center mb-3 relative z-10 flex-grow items-center">
      ...
      {/* Goal Title */}
      <h3 className={`text-base font-semibold mb-2 text-center relative z-10 leading-tight px-1 ${
        isCompleted ? 'text-white' : ''
      }`}>{goal.title}</h3>

      {/* Progress Text */}
      <p className={`text-xs text-center mb-3 flex items-center justify-center gap-1 relative z-10 px-1 ${
        isCompleted
          ? 'text-white/90'
        ...
        {isCompleted && (
          <motion.span
            ...
            className="text-sm filter drop-shadow-lg ml-1 flex items-center"
          >
            {justCompleted ? (
